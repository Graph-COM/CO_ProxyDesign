...
class PNA_concave(torch.nn.Module):
    def __init__(self):
        super().__init__()
        deg_file = torch.load(' path to deg').to(device)# add path to the deg file
    ...
    def forward(self, x, edge_index, batch):
        ...
        x_combine = self.mlp_1(x_combine)
        x_combine = 20 - x_combine.relu()
        x_combine = self.mlp_2(x_combine)
        x_out = x_combine + x_lin
        return x_out


class PNA_concave2(torch.nn.Module):
    def __init__(self):
        super().__init__()
        deg_file = torch.load(' path to deg').to(device)# add path to the deg file
        #self.node_emb = Embedding(11, 75)
