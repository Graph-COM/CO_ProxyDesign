...
def find_min_score(probs):
    ...
    for i in probs:
        ...
        if sum(i)<8 or sum(i) > 8:
            ...
            if punishment == 0 and y < min_score:
                min_score = y
                target_edge_feature = edge_feature
            if punishment == 0 and y > max_score:
                max_score = y
            
    return node_feature, node_feature_label, target_edge_feature, min_score, max_score
...
def make_an_instance(save_path,index):
    ...
    node_feature, node_feature_label, edge_feature, score, max_score = find_min_score(typed_probs)
    print(score)
    print(max_score)
    ...
    list_to_save = {'node_feature':node_feature, 'node_feature_label': node_feature_label, 'edge_feature': edge_feature, 'score': score, 'max_score': max_score }
...
def main():
    ...
    save_path = './testset_matching/'
    for index in tqdm(range(1,501)):
        print("generating the "+str(index)+" instance!")
        make_an_instance(save_path,index)
        print("mean min score: "+str(np.mean(min_score_list)))
        print("mean max score: "+str(np.mean(max_score_list)))
