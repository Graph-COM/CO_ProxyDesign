...
from train_g.model import Model_Mnist2test, ResBlock
from train_g.model import Model_Mnist
import pickle
...
def test_an_instance(predictor, model, edge_index, node_feature, node_feature_label, penalty_coefficient):
    ...
    for i in range(24):
        ...
        punishment_0 = punishment_0 + penalty_0 * penalty_coefficient
        ...
        punishment_1 = punishment_1 + penalty_1 * penalty_coefficient
...
def main():
    ...
    for index in tqdm(range(1,501)):
        ...
        num_match = num_match + match_flag
        if match_flag == 1 and score_by_model < min_score:
            min_score = score_by_model
            this_model_rounded = model_out_rounded

        if min_score < 10000:
            gumbel_list.append(min_score)
        ...
        print("instance "+str(index)+"this_score: "+str(min_score))
        print("instance "+str(index)+"gt_score: "+str(score))
        print("instance "+str(index)+"model out rounded: "+str(this_model_rounded))
