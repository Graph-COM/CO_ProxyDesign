...
from model_con import Con_Mnist2, ResBlock, Con_Mnist
from loss_con import ErdosLoss
...
def train(model, train_loader,criterion_1, criterion_2, optimizer,device, schedular):
    ...
    for data in train_loader:
        ...
        res_out, edge_feature = model(x_input,data.edge_index,the_edge_attr)
...
def main():
    ...
    parser.add_argument('--proxy_path', dest = 'proxy_path',default = '../train_con/train_files/con/train_con_lr0005/best_val_model.pth')
    ...
    proxy = Con_Mnist(Res_Block = ResBlock, in_channels = 16, hidden_channels = 32, out_channels = 128)
    ...
    for epoch in tqdm(range(1,202)):
        ...
        if (val_mse_optimizer<best_mse_val):
        ...
        print('Epoch:'+str(epoch) +'Train MSE Opt: '+str(train_mse_optimizer.item())+'Val MSE Opt: '+str(val_mse_optimizer.item())+'Train MSE Res: '+str(train_mse_res.item())+'Val MSE Res: '+str(val_mse_res.item()))
    writer.close()
