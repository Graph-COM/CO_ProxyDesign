...
def main():
    ...
    model = Aff_Mnist(Res_Block = ResBlock, in_channels = 16, hidden_channels = 32, out_channels = 128)
    ...
    for epoch in tqdm(range(1,202)):
        ...
        print(f'Epoch: {epoch:03d}, Train MSE Pre: {train_mse_predict:.4f}, Test MSE Pre: {val_mse_predict:.4f},Train MSE Res: {train_mse_res:.4f}, Test MSE Res: {val_mse_res:.4f} ')
    writer.close()
