...
class SAGE_edge_linear(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels,out_channels):
        super(SAGE_edge_linear, self).__init__()
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        node_feature_96 = node_feature_96
        node_feature_32 = self.lin32(node_feature_32)
        node_feature_32 = node_feature_32
        ...
        edge_out_con = 100 - edge_out_con.relu()
        edge_out_con = self.lin_64_1(edge_out_con)
        edge_out = edge_out + edge_out_con
...
class SAGE_edge_linear2(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels,out_channels):
        super(SAGE_edge_linear2, self).__init__()
        torch.manual_seed(12345)
...
class Linear_Mnist(torch.nn.Module):
    def __init__(self, Res_Block, in_channels, hidden_channels,out_channels):
        ...
        self.SAGE_edge_linear = SAGE_edge_linear(in_channels, hidden_channels, out_channels)

    def forward(self, x, edge_index, edge_attr, batch):
        res_out, sage_input = self.ResNet(x)
        #import pdb; pdb.set_trace()
        predict = self.SAGE_edge_linear(sage_input, edge_index, edge_attr, batch)
        return res_out, predict


class Linear_Mnist2(torch.nn.Module):
    def __init__(self, Res_Block, in_channels, hidden_channels,out_channels):
        super(Linear_Mnist2, self).__init__()
        torch.manual_seed(12345)
        self.ResNet = ResNet(Res_Block)
        self.SAGE_edge_linear2 = SAGE_edge_linear2(in_channels, hidden_channels, out_channels)

    def forward(self, x, edge_index, edge_attr):
        res_out, sage_input = self.ResNet(x)
        #import pdb; pdb.set_trace()
        predict = self.SAGE_edge_linear2(sage_input, edge_index, edge_attr)
        return res_out, predict