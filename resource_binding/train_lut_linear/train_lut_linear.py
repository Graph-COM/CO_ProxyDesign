...
from model import PNA_linear, LINEAR_lut, weightConstraint
...
def train(model, train_loader,criterion, optimizer,device):
    ...
    for data in train_loader:
        data.to(device)
        predict_label = data.predict_lut.reshape(-1,1).float()
        out = model(data.x, data.edge_index, data.batch)
        loss = criterion(out, predict_label)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        constraints=weightConstraint()
        model._modules['lin32_1'].apply(constraints)
...
def main():
    ...
    old_train_path = r'./train_lut_linear.py'
    new_train_path = os.path.join(args.save_path,'train_lut_linear.py')
    ...
    data_splits = dataset.get_idx_split(split_type = None)
    ...
    model = LINEAR_lut(in_channels = 10, hidden_channels = 128, out_channels = 512)
    constraints=weightConstraint()
    model._modules['lin32_1'].apply(constraints)
