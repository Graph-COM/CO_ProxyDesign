...
from model import PNA_linear, LINEAR_dsp, weightConstraint
...
def train(model, train_loader,criterion, optimizer,device):
    ...
    for data in train_loader:
        data.to(device)
        #print(model._modules['lin32_1'].weight)
        predict_label = data.predict_dsp.reshape(-1,1).float()
...
def main():
    ...
    old_train_path = r'./train_dsp_linear.py'
    new_train_path = os.path.join(args.save_path,'train_dsp_linear.py')
    ...
    #model = PNA_linear()
    model = LINEAR_dsp(in_channels = 10, hidden_channels = 128, out_channels = 512)
    constraints=weightConstraint()
    model._modules['lin32_1'].apply(constraints)
