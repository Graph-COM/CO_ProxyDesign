...
for k in range(len(case_index)):
    ...
    for i in range(len(lines)):
        ...
        if lines[i][0:2]=='in' and 'm' not in lines[i] and 'o' not in lines[i]: 
            ...
            if lines[i][3]==' ': #if the node number is only single like x
                node_name=node_name+[lines[i][0:3]]  # node_name = node name + inx
                precision=int(lines[i][7:9]) # precision = x (after INT)
                p_binary=to_binary(precision)  # transfer the precision to binary
                node_features=node_features+[[1,0,0,0,p_binary[0],p_binary[1],p_binary[2],p_binary[3],p_binary[4],0,0]] # node_features = node_features+[1,0,0,0+binary+0,0]
            elif lines[i][4]==' ': #if the node number is xx
            ...
            elif lines[i][5]==' ':#if the node number is xxx
                node_name=node_name+[lines[i][0:5]]
                precision=int(lines[i][9:11])
                p_binary=to_binary(precision)
                node_features=node_features+[[1,0,0,0,p_binary[0],p_binary[1],p_binary[2],p_binary[3],p_binary[4],0,0]]
            else:  #if the node number is xxxx
        ...
        if lines[i][0]=='m' and ('*' in lines[i] or '+' in lines[i]):
            ...
            if lines[i][2]==' ' and lines[i][4]==' ': #if the node number is only single like x
                node_name=node_name+[lines[i][0:2]] # node_name = node name + mx
                precision=int(lines[i][8:10])
                p_binary=to_binary(precision)
                if lines[i][4]=='+': #if '+' it is 0,1,0,0,...,0,0
                    ...
                    node_dir=node_dir+[node_count-1] # if '*', record the index in node_dir
                node_number_mapping[int(lines[i][1:2])]=node_count-1
            if lines[i][3]==' ' and lines[i][5]==' ': #if the node number is only single like xx
            ...
            if lines[i][4]==' ' and lines[i][2]!=' ': #if the node number is only single like xxx
                node_name=node_name+[lines[i][0:4]]
                precision=int(lines[i][10:12])
                p_binary=to_binary(precision)
                if lines[i][5]=='+':
            ...
            if lines[i][5]==' ' and lines[i][3]!=' ':#if the node number is only single like xxxx
                node_name=node_name+[lines[i][0:5]]
    ...
    LUT_number=case.iloc[3]
    DSP_number=case.iloc[5]
    ...
    for i in range(len(LUT_number)):
        ...
        DSP[i]=DSP_number[i]
        CP[i]=CP_number[i]
        LUT_dir[i]=LUT_op[i]  # get the LUT,DSP...of each solution
    ...
    df.to_csv('case_'+str(case_index[k])+'/case_'+str(case_index[k])+'_plain.csv',index=False)  # feature of in and m
    ...
    for i in range(len(LUT_dir)):
        ...
        for j in range(len(solution)):
            index=node_number_mapping[solution[j]] # use node name node_number_mapping[xx after in] = which row
            if index in node_dir_set: # if index has multiplication
                f10[index]=10
        df['f10']=f10
