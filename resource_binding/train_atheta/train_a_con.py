...
from model import LINEAR_model2, PNA_linear2
...
def train(model, train_loader,criterion, optimizer,device):
    ...
    for data in train_loader:
        data.to(device)
        alpha = torch.randint(1,801, (torch.max(data.batch)+1, 1))/10
...
def main():
    ...
    #model = PNA_linear2()
    model = LINEAR_model2(in_channels = 11, hidden_channels = 128, out_channels = 512)
